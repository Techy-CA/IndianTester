    ...
    <div className="min-h-screen w-full bg-[#f0fdf4] flex flex-col items-center justify-center px-4 py-10">
      ...
      <div className="w-full max-w-[400px] rounded-3xl border-2 border-black bg-[#86efac] p-7 space-y-5"
        ...
        <div className="flex flex-col items-center space-y-3 pb-2">
          <div className="w-16 h-16 rounded-2xl bg-yellow-400 border-2 border-black flex items-center justify-center">
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-black/50 uppercase tracking-widest">Email</label>
          <input type="email" value={email}
            placeholder="you@example.com"
            onChange={e => setEmail(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleLogin()}
            className="w-full px-4 py-3 rounded-xl border-2 border-black text-sm text-gray-800 placeholder:text-gray-300 outline-none bg-white transition-all"
        ...
        {/* Password */}
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-black/50 uppercase tracking-widest">Password</label>
          <div className="relative">
            <input type={showPassword ? 'text' : 'password'} value={password}
              placeholder="••••••••"
              onChange={e => setPassword(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleLogin()}
              className="w-full px-4 py-3 pr-12 rounded-xl border-2 border-black text-sm text-gray-800 placeholder:text-gray-300 outline-none bg-white transition-all"
              style={{ boxShadow: '2px 2px 0px 0px #000000' }}
              onFocus={e => e.target.style.boxShadow = '3px 3px 0px 0px #000000'}
              onBlur={e => e.target.style.boxShadow = '2px 2px 0px 0px #000000'}
            />
        ...
        <button onClick={handleLogin} disabled={loading}
          className="w-full py-3 rounded-xl text-sm font-black transition-all flex items-center justify-center gap-2 border-2 border-black"
          style={{
        ...
        <p className="text-center text-xs text-black/50">
          No account?{' '}
          <Link to="/register" className="text-black font-black underline underline-offset-2 hover:text-green-900 transition-colors">
            Create account
          </Link>
        </p>
      </div>
      ...
      <div className="mt-5 flex items-center gap-2">
        ...
        <Link to="/admin/login"
          className="text-[11px] text-black/30 hover:text-black/60 transition-colors px-3 py-1.5 rounded-full border border-black/10 hover:border-black/30 flex items-center gap-1.5">
