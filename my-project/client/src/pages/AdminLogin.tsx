  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.data.role !== 'admin' && res.data.role !== 'superadmin') {
        setError('Access denied. Admins only.'); setLoading(false); return;
      }
      ...
      <div className="w-full max-w-[400px] rounded-3xl border-2 border-black bg-[#c4b5fd] p-7 space-y-5"
        ...
        <div className="flex flex-col items-center space-y-3 pb-2">
          ...
          <div className="text-center">
            <h1 className="text-xl font-black text-black tracking-tight">Admin Portal</h1>
            <p className="text-xs text-black/50 mt-0.5">Sign in to manage your platform</p>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="admin@indiantester.com"
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input
              ...
              onKeyDown={e => e.key === 'Enter' && handleLogin()}
              className="w-full px-4 py-3 pr-12 rounded-2xl border-2 border-black text-sm text-gray-800 placeholder:text-gray-300 outline-none bg-white"
              style={{ boxShadow: '2px 2px 0px 0px #000000' }}
              onFocus={e => e.target.style.boxShadow = '3px 3px 0px 0px #000000'}
              onBlur={e => e.target.style.boxShadow = '2px 2px 0px 0px #000000'}
            />
        ...
        <p className="text-center text-xs text-black/50">
          New admin?{' '}
          <Link to="/admin/register" className="text-black font-bold underline underline-offset-2 hover:text-purple-900 transition-colors">
            Create account
          </Link>
        </p>
        <p className="text-center text-[11px] text-black/30">
          Not an admin?{' '}
          <Link to="/login" className="underline underline-offset-2 hover:text-black/60 transition-colors">
            Buyer Login
          </Link>
        </p>
      </div>
