      ...
      <nav className="w-full bg-white border-b-2 border-black px-4 py-3 flex items-center justify-between sticky top-0 z-50"
        ...
        <div className="flex items-center gap-2">
          <Link to="/buyer/profile"
            className="flex items-center gap-2 px-2.5 py-1.5 rounded-full border-2 border-black bg-white hover:bg-[#86efac] transition-all"
            ...
            <span className="text-xs font-bold text-black hidden sm:block max-w-[80px] truncate">{name}</span>
          </Link>
          <button onClick={handleLogout}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border-2 border-black bg-white text-xs font-black text-black hover:bg-red-50 hover:border-red-400 hover:text-red-600 transition-all"
            style={{ boxShadow: '2px 2px 0px 0px #000000' }}>
            <IconLogout />
      ...
      <main className="w-full max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="rounded-2xl border-2 border-black bg-[#86efac] p-5"
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-black/40 uppercase tracking-widest">Welcome back</p>
              <h1 className="text-lg font-black text-black tracking-tight leading-tight">Hello, {name}!</h1>
              <p className="text-[11px] text-black/50 mt-0.5">Here's your account overview.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {stats.map(({ icon, label, value, bg }) => (
            <div key={label}
              className="rounded-2xl border-2 border-black p-3 flex flex-col gap-2"
              style={{ backgroundColor: bg, boxShadow: '3px 3px 0px 0px #000000' }}>
              <div className="w-7 h-7 rounded-lg bg-black/10 border border-black/15 flex items-center justify-center text-black">
                {icon}
              </div>
              <div>
        ...
        <div className="rounded-2xl border-2 border-black bg-white p-4 space-y-3"
          style={{ boxShadow: '4px 4px 0px 0px #000000' }}>
          <p className="text-[10px] font-black text-black/30 uppercase tracking-widest">Quick Actions</p>
          <div className="grid grid-cols-1 gap-2.5">
            {actions.map(({ icon, label, desc, to }) => (
              <Link key={label} to={to}
                className="flex items-center justify-between px-4 py-3.5 rounded-xl border-2 border-black bg-[#f0fdf4] hover:bg-[#86efac] transition-all group"
                style={{ boxShadow: '2px 2px 0px 0px #000000' }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-black/40">{desc}</p>
                  </div>
                </div>
                <span className="text-black/25 group-hover:text-black transition-colors shrink-0">
                  <IconChevron />
                </span>
              </Link>
            ))}
